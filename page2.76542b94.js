document.querySelector(".catalog-cntr");const t="https://tech-showcase-store.onrender.com/api";async function e(){try{let e=await fetch("${API_BASE_URL}/products");if(!e.ok)throw Error(`Error HTTP: ${e.status}`);let c=await e.json();!function(e){let c=document.querySelector(".catalog-cntr");if(!c)return console.error("Error: container .catalog-cntr not found");console.log("RENDER"),c.innerHTML="",e.forEach(t=>{let e=`
      <article class="product">
        ...
        <article class="product">
          ...
          <a href="#" class="product__button" data-qa="product-hover">Buy</a>
        </article>`;r.insertAdjacentHTML("beforeend",e)})}catch(t){console.error("Ошибка при фильтрации:",t)}}})}),document.addEventListener("DOMContentLoaded",function(){document.querySelectorAll(".product__buy-btn").forEach(e=>{e.addEventListener("click",async e=>{let r=e.target.getAttribute("data-product-id");try{if(!(await fetch("${API_BASE_URL}/cart/add",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({productId:r})})).ok)throw Error("Error adding product to cart");t()}catch(t){console.error("Error adding to cart:",t)}})});async function t(){try{let t=await fetch("${API_BASE_URL}/cart"),e=await t.json(),r=document.querySelector(".cart__items");r.innerHTML="",e.forEach(t=>{let e=document.createElement("div");e.classList.add("cart__item"),e.innerHTML=`
        ...
        `,r.appendChild(e)});let c=e.reduce((t,e)=>t+e.product.price*e.quantity,0);document.querySelector(".cart__total-value").textContent=`$${c.toFixed(2)}`}catch(t){console.error("Error updating cart:",t)}}});
